...
#ifdef FTU_CNT_SEL_EXIST
/**
 * @brief FTU Counter Select enumeration
 *
 */
typedef enum
{
	FTU_CNT_SEL_INTERNAL_CNT		= 0U,	/*!< Select the FTU internal counter (FTU_SC.CLKS) */
	FTU_CNT_SEL_EXTERNAL_CNT		= 1U,	/*!< Select an external counter as FTU counter (EFTU_CNT) */
} FTU_CounterSelectType;
#endif
...
LOCAL_INLINE void FTU_HWA_ClearModuleRegister(FTU_Type *pFtu)
{
    ...
    pFtu->CNTIN = 0U;
    pFtu->STATUS = pFtu->STATUS & (~FTU_STATUS_MASK);
    ...
    pFtu->SWOCTRL = 0U;
    pFtu->PWMLOAD = 0U;
    pFtu->PAIR0DEADTIME = 0U;
    pFtu->PAIR1DEADTIME = 0U;
    pFtu->PAIR2DEADTIME = 0U;
    pFtu->PAIR3DEADTIME = 0U;
}
...
#ifdef FTU_FDUTYCTL_EXIST
/**
 * @brief Enable channel interrupt when CPWM full duty cycle
 *
 * @param pFtu the base address of the FTU instance
 */
LOCAL_INLINE void FTU_HWA_EnableFullDutyIntrEnable(FTU_Type *pFtu)
{
	pFtu->SC |= FTU_SC_FDUTYCTL_MASK;
}

/**
 * @brief Disable channel interrupt when CPWM full duty cycle
 *
 * @param pFtu the base address of the FTU instance
 */
LOCAL_INLINE void FTU_HWA_DisableFullDutyIntrEnable(FTU_Type *pFtu)
{
	pFtu->SC &= ~FTU_SC_FDUTYCTL_MASK;
}
#endif
...
{
	u32Channel &= (~1u);
    ((volatile uint32_t *)(&pFtu->PAIR0DEADTIME))[u32Channel] =
    			((((volatile uint32_t *)(&pFtu->PAIR0DEADTIME))[u32Channel]) & (~(uint32_t)FTU_DEADTIME_DTPS_MASK))
...
LOCAL_INLINE void FTU_HWA_ConfigPairDeadtimeValue(FTU_Type *pFtu, uint32_t u32Channel, uint32_t u32Value)
{
	u32Channel &= (~1u);
	((volatile uint32_t *)(&pFtu->PAIR0DEADTIME))[u32Channel] =
			((((volatile uint32_t *)(&pFtu->PAIR0DEADTIME))[u32Channel]) & (~(uint32_t)(FTU_DEADTIME_DTVAL_MASK | FTU_DEADTIME_DTVALEX_MASK)))
...
{
    return (0u != (pFtu->CONTROLS[u8Channel].CSCn & (uint32_t)FTU_CnSC_CHF_MASK));
}
...
{
	pFtu->CONTROLS[u8Channel].CSCn = 	(pFtu->CONTROLS[u8Channel].CSCn & (~(uint32_t)FTU_CnSC_TRIGMODE_MASK))
									 | 	(uint32_t)FTU_CnSC_TRIGMODE(eMode);
}
#ifdef FTU_FAULT_DIS_DELAY_FEATURE
/**
...
{
	pFtu->CONTROLS[u8Channel].CSCn = 	(pFtu->CONTROLS[u8Channel].CSCn & (~(uint32_t)FTU_CnSC_FDLYSEL_MASK))
									 | 	(uint32_t)FTU_CnSC_FDLYSEL(eSelection);
}
#endif
/**
...
{
	return (uint8_t)((pFtu->CONTROLS[u8Channel].CSCn & (uint32_t)FTU_CnSC_CHIE_MASK) >> FTU_CnSC_CHIE_SHIFT);
}
...
#ifdef FTU_CNT_SEL_EXIST
/**
 * @brief Select the FTU counter
 *
 * @param pFtu the base address of the FTU instance
 * @param eCntSel FTU Counter select
 */
LOCAL_INLINE void FTU_HWA_ConfigFTUCntSel(FTU_Type *pFtu, FTU_CounterSelectType eCntSel)
{
	pFtu->SC = (pFtu->SC & (~(uint32_t)FTU_SC_FTUCNTSEL_MASK)) | FTU_SC_FTUCNTSEL(eCntSel);
}
#endif
...
{
#if defined(FTU_SC_TCLKSEL_MASK)
	pFtu->SC = (pFtu->SC & (~(uint32_t)FTU_SC_TCLKSEL_MASK))
			 | FTU_SC_TCLKSEL(eTclk);
#elif defined(FTU_SC_TCKSEL_MASK)
	pFtu->SC = (pFtu->SC & (~(uint32_t)FTU_SC_TCKSEL_MASK))
				 | FTU_SC_TCKSEL(eTclk);
#else
#error "No valid FTU_SC_TC*KSEL_MASK definition"
#endif
}
#endif
...
LOCAL_INLINE void FTU_HWA_ConfigChannelMode(FTU_Type *pFtu, uint8_t u8Channel, FTU_ChannelModeType eMode)
{
	pFtu->CONTROLS[u8Channel].CSCn = 	(		pFtu->CONTROLS[u8Channel].CSCn
											& 	(~(uint32_t)(FTU_CnSC_MSB_MASK | FTU_CnSC_MSA_MASK)))
									 | 	(		((uint32_t)eMode << FTU_CnSC_MSA_SHIFT)
											& 	((uint32_t)(FTU_CnSC_MSB_MASK | FTU_CnSC_MSA_MASK)));
}

LOCAL_INLINE void FTU_HWA_ConfigChannelEdgeLevel(FTU_Type *pFtu, uint8_t u8Channel, FTU_ChannelEdgeLevelType eEdgeLevel)
{
	pFtu->CONTROLS[u8Channel].CSCn = 	(pFtu->CONTROLS[u8Channel].CSCn & (~(uint32_t)(FTU_CnSC_ELSB_MASK | FTU_CnSC_ELSA_MASK)))
									 | 	(((uint32_t)eEdgeLevel << FTU_CnSC_ELSA_SHIFT) & (uint32_t)(FTU_CnSC_ELSB_MASK | FTU_CnSC_ELSA_MASK));
}
#ifdef FTU_ICM_FEATURE
LOCAL_INLINE uint8_t FTU_HWA_GetEdgeNumberCount(FTU_Type *pFtu, uint8_t u8Channel)
{
	return (uint8_t)((pFtu->CONTROLS[u8Channel].CSCn & FTU_CnSC_ICEXP_NUM_ICM_ECNT_MASK) >> FTU_CnSC_ICEXP_NUM_ICM_ECNT_SHIFT);
}

LOCAL_INLINE void FTU_HWA_ConfigEdgeNumber(FTU_Type *pFtu, uint8_t u8Channel, uint8_t u8EdgeNumber)
{
	pFtu->CONTROLS[u8Channel].CSCn = 	(	pFtu->CONTROLS[u8Channel].CSCn
											& ((uint32_t)(~FTU_CnSC_ICEXP_NUM_ICM_ECNT_MASK)))
									| 	FTU_CnSC_ICEXP_NUM_ICM_ECNT(u8EdgeNumber);
}
...
{
	pFtu->CONTROLS[u8Channel].CSCn |= (uint32_t)FTU_CnSC_ICM_SINGLE_MASK;
}
/**
...
{
	pFtu->CONTROLS[u8Channel].CSCn |= (uint32_t)FTU_CnSC_ICM_CONT_MASK;
}
/**
...
{
	pFtu->CONTROLS[u8Channel].CSCn |= (uint32_t)FTU_CnSC_ICDPM_START_MASK;
}

LOCAL_INLINE void FTU_HWA_ConfigInputCaptureMeasureMode(FTU_Type *pFtu, uint8_t u8Channel, FTU_MeasurementModeType eMode)
{
	pFtu->CONTROLS[u8Channel].CSCn = 	(pFtu->CONTROLS[u8Channel].CSCn & (~(uint32_t)FTU_CnSC_ICM_MODE_MASK))
									 | 	FTU_CnSC_ICM_MODE(eMode);
}
...
{
	return (FTU_MeasurementModeType)((pFtu->CONTROLS[u8Channel].CSCn & FTU_CnSC_ICM_MODE_MASK) >> FTU_CnSC_ICM_MODE_SHIFT);
}
#endif
/**
...
{
    pFtu->CONTROLS[u8Channel].CSCn =  (pFtu->CONTROLS[u8Channel].CSCn & (~(uint32_t)FTU_CnSC_ELSA_MASK))
    								| ((uint32_t)(FTU_CnSC_MSB_MASK | FTU_CnSC_ELSB_MASK));
}
...
{
    pFtu->CONTROLS[u8Channel].CSCn |= (uint32_t)(FTU_CnSC_MSB_MASK | FTU_CnSC_ELSA_MASK);
}
...
{
    pFtu->CONTROLS[u8Channel].CSCn =  (		pFtu->CONTROLS[u8Channel].CSCn
    									& 	(~(uint32_t)(FTU_CnSC_ELSA_MASK | FTU_CnSC_MSB_MASK | FTU_CnSC_MSA_MASK)))
    								| ((uint32_t)FTU_CnSC_ELSB_MASK);
}
...
{
    pFtu->CONTROLS[u8Channel].CSCn |= (uint32_t)FTU_CnSC_CHIE_MASK;
}
...
{
    pFtu->CONTROLS[u8Channel].CSCn |= (uint32_t)FTU_CnSC_DMA_MASK;
}
...
{
    pFtu->CONTROLS[u8Channel].CSCn &= ~(uint32_t)FTU_CnSC_CHF_MASK;
}
