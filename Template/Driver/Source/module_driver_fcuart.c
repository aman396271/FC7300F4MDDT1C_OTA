 ...
 *   0.2.0      2024-10-12    Flagchip055     N/A          Fix Misra-c issue
...
FCUART_ErrorType FCUART_Init(FCUART_HandleType *pUartHandle, FCUART_InitType *pInitCfg)
{
    ...
    {
#endif
        FCUART_ErrorType tRetVal = FCUART_ERROR_FAILED;
        ...
        if ((FCUART_ErrorType)FCUART_ERROR_OK == FCUART_LL_ProcessBaud(u32Smb, &u32OverSamp, &u32Sbr))
        {
            ...
            u32TempModir = (uint32_t)0U                 |   /* initial value                                          */
                            FCUART_HFCR_RXRTSCFG(0U)    |   /* Receive RTS Configuration                              */
                            FCUART_HFCR_TXCTSSRC(0U)    |   /* Transmit CTS Source                                    */
                            FCUART_HFCR_TXCTSCFG(0U)    |   /* Transmit CTS Configuration                             */
                            FCUART_HFCR_RXRTSEN(0U)     |   /* Receiver Request-to-Send Enable                        */
                            FCUART_HFCR_TXRTSPOL(0U)    |   /* Transmitter Request-to-Send Polarity                   */
                            FCUART_HFCR_TXRTSEN(0U)     |   /* Transmitter Request-to-Send Enable                     */
                            FCUART_HFCR_TXCTSEN(0U);        /* Transmitter Clear-to-Send Enable                       */
...
void FCUART_StopTransmit(FCUART_HandleType *pUartHandle)
{
    ...
    if(pUartHandle->eInstance >= FCUART_INSTANCE_COUNT)
    {
        FCUART_ReportDevError(FCUART_STOP_TRANSMIT_ID, FCUART_E_PARAM_INSTANCE);
    }
    else
    {
#endif
...
void FCUART_SendData_9Bits(FCUART_HandleType *pUartHandle, uint16_t u16Data)
{
    ...
    if(pUartHandle->eInstance >= FCUART_INSTANCE_COUNT)
    {
        FCUART_ReportDevError(FCUART_SENDDATA_9BITS, FCUART_E_PARAM_INSTANCE);
    }
    else
    {
#endif
...
void FCUART_GetData_9Bits(FCUART_HandleType *pUartHandle, uint16_t *pData)
{
    ...
    if(pUartHandle->eInstance >= FCUART_INSTANCE_COUNT)
    {
        FCUART_ReportDevError(FCUART_GETDATA_9BITS, FCUART_E_PARAM_INSTANCE);
    }
    else
    {
#endif
        ...
        /* Get 0 ~ 8 bits */
        *pData = (uint16_t)(((uint16_t)u8SingleBit_8 << 8U) | (uint16_t)u8Temp);
...
void FCUART_SendData_10Bits(FCUART_HandleType *pUartHandle, uint16_t u16Data)
{
    ...
    if(pUartHandle->eInstance >= FCUART_INSTANCE_COUNT)
    {
        FCUART_ReportDevError(FCUART_SENDDATA_10BITS, FCUART_E_PARAM_INSTANCE);
    }
    else
    {
#endif
...
void FCUART_GetData_10Bits(FCUART_HandleType *pUartHandle, uint16_t *pData)
{
    ...
    if(pUartHandle->eInstance >= FCUART_INSTANCE_COUNT)
    {
        FCUART_ReportDevError(FCUART_GETDATA_10BITS, FCUART_E_PARAM_INSTANCE);
    }
    else
    {
#endif
        ...
        u8Temp  = FCUART_HWA_GetData(pUart);
        /* Get 0 ~ 9 bits */
        *pData = (uint16_t)(((uint16_t)u8SingleBit_8 << 8U) | ((uint16_t)u8SingleBit_9 << 9U) | (uint16_t)u8Temp);
                                            ...
                                            bool bParityEnable)
{
    ...
    if(pUartHandle->eInstance >= FCUART_INSTANCE_COUNT)
    {
        FCUART_ReportDevError(FCUART_SETBITMODEANDPARITY, FCUART_E_PARAM_INSTANCE);
    }
    else
    {
#endif
